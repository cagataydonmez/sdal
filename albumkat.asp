...
if len(session("eskiyeni")) = 0 then
ey = request.querystring("ey")
if ey = "eski" then
session("eskiyeni") = "eski"
else
session("eskiyeni") = "yeni"
end if
end if
...
<td align=right>
<%if session("eskiyeni") = "eski" then%>
<a href=albumkat.asp?kat=<%=kas("id")%>&sf=<%=sf%>&ey=yeni>Yeni Görünüme Geç</a>
<%else%>
<a href=albumkat.asp?kat=<%=kas("id")%>&sf=<%=sf%>&ey=eski>Eski Görünüme Dön</a>
<%end if%>
</td></tr></table>
</td>
</tr>
...
<% if session("eskiyeni") = "yeni" then%>

<a href="kucukresim3.asp?r=<%=ks("dosyaadi")%>" rel="lightbox[sayfa]" title="<%=ks("baslik")%>">
<img src="kucukresim.asp?iheight=100&r=<%=ks("dosyaadi")%>" border=1><br><font style="font-size:9;font-weight:bold;color:navy;">Büyüt</font>
</a>
