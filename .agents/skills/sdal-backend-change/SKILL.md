...
3. Inspect the route registration in `server/appRuntime.js` only as needed.
4. Read the specific `server/routes/*`, `server/src/services/*`, `server/src/repositories/*`, or `server/src/infra/*` sections involved.
5. Before changing API request/response shape, search Flutter and React clients for endpoint and field usage.
...
- Use `sed -n` for focused ranges after locating symbols.
...
- Do not change backend response shape without checking Flutter `ApiClient` repositories and web call sites.
- Treat `server/db.js`, migrations, auth/session, uploads, and admin permissions as fragile.
- Do not run destructive DB/deploy commands without explicit approval.
...
- Use endpoint-specific contract tests when present.
