...
if [[ $# -eq 0 ]]; then
  printf 'Usage: %s "<watchOS/TestFlight task>"\n' "$(basename "$0")" >&2
  exit 64
fi
...
prompt="Use the sdal-watchos-change skill.
...
- Summarize changed files, checks, signing/TestFlight risk, and unverified items.
- Do not commit."
