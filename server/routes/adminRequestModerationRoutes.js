export function registerAdminRequestModerationRoutes(app, {
  ...
  app.get('/api/new/admin/requests', requireModerationPermission('requests.view'), async (req, res) => {
    try {
  ...
  app.post('/api/new/admin/requests/:id/review', requireModerationPermission('requests.moderate'), async (req, res) => {
    try {
  ...
  app.get('/api/new/admin/teacher-network/links', requireModerationPermission('requests.view'), async (req, res) => {
    try {
      ensureTeacherAlumniLinksTable();
      ensureTeacherAlumniLinkModerationEventsTable();
      const actor = req.authUser || getCurrentUser(req);
  ...
  app.post('/api/new/admin/teacher-network/links/:id/review', requireModerationPermission('requests.moderate'), async (req, res) => {
    try {
      ensureTeacherAlumniLinksTable();
      ensureTeacherAlumniLinkModerationEventsTable();
      ...
      const affectedSiblingIds = await sqlAllAsync(
        `SELECT id
  ...
  app.post('/api/new/admin/verify', requireAdmin, async (req, res) => {
    try {
      ...
      if (!target) return;
      await sqlRunAsync('UPDATE uyeler SET verified = ?, verification_status = ? WHERE id = ?', [value, value === 1 ? 'verified' : 'pending', userId]);
