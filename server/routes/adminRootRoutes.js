...
export function registerAdminRootRoutes(app, {
  ...
  let activityEventsTableReady = false;
  async function ensureActivityEventsTable() {
    if (activityEventsTableReady) return;
    try {
      if (isPostgres) {
        await sqlRunAsync(
      ...
      await sqlRunAsync('CREATE INDEX IF NOT EXISTS idx_user_activity_events_actor_time ON user_activity_events (actor_user_id, occurred_at DESC)');
      await sqlRunAsync('CREATE INDEX IF NOT EXISTS idx_user_activity_events_target_time ON user_activity_events (target_type, target_id, occurred_at DESC)');
      await sqlRunAsync('CREATE INDEX IF NOT EXISTS idx_user_activity_events_type_time ON user_activity_events (event_type, occurred_at DESC)');
      activityEventsTableReady = true;
    } catch (err) {
      writeAppLog?.('warn', 'root_activity_events_table_ensure_failed', {
        message: err?.message || String(err)
      });
    }
  }
  ...
  async function readRootActivitySnapshot(userId) {
    ...
    const [posts, comments, postLikes, photos, photoComments, photoLikes, follows, messages, profileViews, photoViews, sessions, timeline] = await Promise.all([
      firstRows('root_activity_posts', [
        ...
        { sql: `SELECT id, ${dateText('created_at')} AS created_at, COALESCE(content, '') AS text, COALESCE(image, '') AS media_url FROM posts WHERE user_id = ? ORDER BY created_at DESC LIMIT 30`, params: [userId] },
      ...
      firstRows('root_activity_comments', [
        ...
        { sql: `SELECT c.id, c.post_id, p.user_id AS owner_user_id, ${dateText('c.created_at')} AS created_at, COALESCE(c.comment, '') AS text, COALESCE(p.content, '') AS target_text FROM post_comments c LEFT JOIN posts p ON p.id = c.post_id WHERE c.user_id = ? ORDER BY c.created_at DESC LIMIT 40`, params: [userId] }
      ]),
      firstRows('root_activity_post_likes', [
        ...
        { sql: `SELECT r.id, r.post_id, p.user_id AS owner_user_id, ${dateText('r.created_at')} AS created_at, 'like' AS reaction_type, COALESCE(p.content, '') AS target_text FROM post_likes r LEFT JOIN posts p ON p.id = r.post_id WHERE r.user_id = ? ORDER BY r.created_at DESC LIMIT 40`, params: [userId] }
      ]),
      firstRows('root_activity_photos', [
        ...
        { sql: `SELECT id, ${dateText('tarih')} AS created_at, COALESCE(baslik, '') AS title, COALESCE(dosyaadi, '') AS file_name, COALESCE(hit, 0) AS view_count FROM album_foto WHERE CAST(ekleyenid AS INTEGER) = CAST(? AS INTEGER) ORDER BY id DESC LIMIT 30`, params: [userId] }
      ]),
      firstRows('root_activity_photo_comments', [
        ...
        { sql: `SELECT c.id, c.fotoid AS photo_id, p.ekleyenid AS owner_user_id, ${dateText('c.tarih')} AS created_at, COALESCE(c.yorum, '') AS text, COALESCE(p.baslik, '') AS target_text, COALESCE(p.dosyaadi, '') AS file_name FROM album_fotoyorum c LEFT JOIN album_foto p ON p.id = c.fotoid WHERE CAST(c.author_user_id AS INTEGER) = CAST(? AS INTEGER) ORDER BY c.id DESC LIMIT 40`, params: [userId] },
        { sql: `SELECT c.id, c.fotoid AS photo_id, p.ekleyenid AS owner_user_id, ${dateText('c.tarih')} AS created_at, COALESCE(c.yorum, '') AS text, COALESCE(p.baslik, '') AS target_text, COALESCE(p.dosyaadi, '') AS file_name FROM album_fotoyorum c LEFT JOIN album_foto p ON p.id = c.fotoid WHERE CAST(c.uyeid AS INTEGER) = CAST(? AS INTEGER) ORDER BY c.id DESC LIMIT 40`, params: [userId] }
      ]),
      firstRows('root_activity_photo_likes', [
        ...
        { sql: `SELECT l.id, l.photo_id, p.ekleyenid AS owner_user_id, ${dateText('l.created_at')} AS created_at, COALESCE(p.baslik, '') AS target_text, COALESCE(p.dosyaadi, '') AS file_name FROM album_photo_likes l LEFT JOIN album_foto p ON p.id = l.photo_id WHERE l.user_id = ? ORDER BY l.created_at DESC LIMIT 40`, params: [userId] }
      ]),
      ...
      firstRows('root_activity_photo_views', [
        { sql: `SELECT e.target_id, COUNT(*) AS count, MIN(e.occurred_at) AS first_seen_at, MAX(e.occurred_at) AS last_seen_at,
                ...
                ORDER BY MAX(e.occurred_at) DESC LIMIT 60`, params: [userId] },
        { sql: `SELECT e.target_id, COUNT(*) AS count, MIN(e.occurred_at) AS first_seen_at, MAX(e.occurred_at) AS last_seen_at,
                       COALESCE(p.baslik, '') AS title, COALESCE(p.dosyaadi, '') AS file_name
                FROM user_activity_events e
                LEFT JOIN album_foto p ON CAST(p.id AS TEXT) = e.target_id
                WHERE e.actor_user_id = ? AND e.event_type = 'photo_view' AND e.target_type = 'photo'
                GROUP BY e.target_id, p.baslik, p.dosyaadi
                ORDER BY MAX(e.occurred_at) DESC LIMIT 60`, params: [userId] }
      ]),
