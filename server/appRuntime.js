...
const EXPLORE_SUGGESTIONS_CACHE_TTL_MS = envInt('EXPLORE_SUGGESTIONS_CACHE_TTL_MS', dbDriver === 'postgres' ? 30000 : 10000);
...
let userActivityEventsTableReady = false;
function ensureUserActivityEventsTable() {
  if (userActivityEventsTableReady) return;
  if (dbDriver === 'postgres') {
    sqlRun(
  ...
  sqlRun('CREATE INDEX IF NOT EXISTS idx_user_activity_events_actor_time ON user_activity_events (actor_user_id, occurred_at DESC)');
  sqlRun('CREATE INDEX IF NOT EXISTS idx_user_activity_events_target_time ON user_activity_events (target_type, target_id, occurred_at DESC)');
  sqlRun('CREATE INDEX IF NOT EXISTS idx_user_activity_events_type_time ON user_activity_events (event_type, occurred_at DESC)');
  userActivityEventsTableReady = true;
}
